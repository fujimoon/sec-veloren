...
impl<'a> Widget for SlotGrid<'a> {
    ...
    fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
        ...
        for (i, (pos, item)) in items.into_iter().enumerate() {
            ...
            if let Some(item) = item {
                ...
                if self.details_mode {
                    ...
                    let col = self.columns as usize;
                    let size = self.columns as usize;
        ...
        if state.active_context_slot.is_some() {
            let actions = ["Use", "Move", "Split", "Drop", "Cancel"];
            ...
            if let Some(index) = event {
                match actions[index] {
                    "Use" => self.slot_manager.use_selected(),
                    "Drop" => self.slot_manager.dropped_selected(),
