...
command-osdungeon-desc = Enter (or exit) the OS Dungeon: a voxel structure mirroring a filesystem directory. Toggles; an optional path argument sets the starting directory on entry (defaults to the server's working directory).
command-osdungeon_probe-desc = Debug tool: report the actual terrain block kind at (your OS Dungeon session's anchor + dx,dy,dz), and append it to the psypher/trace debug log.
