...
void lod_voxels(vec3 f_pos, vec3 f_norm, vec3 cam_to_frag, out vec3 voxel_pos, out vec3 voxel_norm, out float voxel_sz, out float f_ao) {
    ...
    f_ao = 1.0;
        ...
        voxel_sz = clamp(exp(floor(log(distance(cam_pos.xy, f_pos.xy) * 0.0001 + noise_2d((f_pos.xy + focus_off.xy) * 0.01) * 0.02) * 3) / 3) * VOXEL_SCALE_FACTOR, 1.0, 128.0);
        ...
        while (t < MARCH_THRESHOLD * voxel_sz) {
            vec3 deltas = (fract((f_pos - cam_dir * t) / voxel_sz) - step(vec3(0), -cam_dir * voxel_sz)) / cam_dir * voxel_sz;
            t += max(min(min(deltas.x, deltas.y), deltas.z), 0.00001);

            voxel_pos = (floor((f_pos - cam_dir * t) / voxel_sz) + 0.5) * voxel_sz;
            #ifdef EXPERIMENTAL_PROCEDURALLODDETAIL
                float surf_depth = lod_voxel_noise(voxel_pos + focus_off.xyz, voxel_sz, f_norm);
            #else
                const float surf_depth = 0.0;
            #endif
            if (dot(voxel_pos - f_pos, -f_norm) < surf_depth) {
                vec3 to_center = abs(voxel_pos - f_pos + cam_dir * t);
